...
def sprd_auth(func, mode=AuthMode.READ):
    def auth_wrapper(*args, **kwargs):
        request = args[0]

        group = models.Group.objects.get(user_groups=request.user.groups.all()[0])
        project = models.Project.objects.get(group=group)
